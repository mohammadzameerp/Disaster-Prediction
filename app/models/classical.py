...
def train_xgb(X_train, y_train):
    clf = XGBClassifier(n_estimators=200, max_depth=6, learning_rate=0.1, subsample=0.9, colsample_bytree=0.9, objective="multi:softprob")
...
def evaluate(clf, X_test, y_test):
    ...
    try:
        ...
        onehot = np.zeros((preds.shape[0], n_classes))
...
def predict_classical(clf, artifacts, features):
    ...
    top_features = importances.tolist() if importances is not None else []
